...
func main() {
	fmt.Println("This example demonstrate mongo db example")
	fmt.Println("Reference http://goinbigdata.com/how-to-build-microservice-with-mongodb-in-golang/")
...
func addBooks(s *mgo.Session) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			if mgo.IsDup(err) {
				ErrorWithJSON(w, "book with this ISBN exists", http.StatusBadRequest)
				return
			}
